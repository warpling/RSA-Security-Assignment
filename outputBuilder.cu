...
void generateBadModuliArray(mpz_t *badModuli, mpz_t *moduli, uint32_t *badModuliFlags, int totalModuliCount) {
    ...
    // Overly ambitious and mildly retarded
    badModuli = (mpz_t *) realloc(badModuli, totalModuliCount * sizeof(mpz_t));
    int badModuliCount = 0;
    uint32_t bitMask = 0;

    int i, bitCtr;
    for (i = 0; i < ceil(totalModuliCount/(1.0*UINT32_LENGTH)); ++i)
    {
        bitMask = 0x80000000;
        for (bitCtr = 0; bitCtr < UINT32_LENGTH; ++bitCtr)
        {
